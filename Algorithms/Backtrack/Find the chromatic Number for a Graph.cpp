...
/// given a Graph find the chromatic number. No two adjacent node are coloured same.
int Graph_Matrix[10][10],chromaticN = -1,color[10],node;

/// finds colour for each node if possible fighting with the constraint
/// to find the minimum colour to paint. We put a highest limit equal to node number
int getnodeColor(int k){

    int i;

    while(1){                                       /// infinite loop not a issue - in the calling function we return at k == node
        color[k] = (color[k]+1) % (node+1);         /// nice modular trick to increase the colour values 0 means colour finished
...
int Coloring(int k){
    ...
    if(k == node)                                /// base case - solution found
        ...
        chromaticN = max(chromaticN,color[k]);
        return Coloring(k+1);                   /// proceeds to the next node
    }
...
int main() {
    ...
    Coloring(0);                                /// Colouring is clearly possible as we set colour to node number
    /// print
    pf("Chromatic Number : %d\n",chromaticN);
    pf("Node Colours : \n");
    nfr(i,0,node){
        pf("%d ",color[i]);
    }
