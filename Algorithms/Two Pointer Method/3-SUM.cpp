...
find three different array values such that their sum is x, or report that no such values exist.
*/
...
int main() {
    ...
    /// for each element we apply the 2-SUM two other 2 elements
    nfr(i,0,n){

        sum = input[i];                                         /// first value set

        /// now it the 2-SUM problem
        ...
        while(p1 < p2){
            ...
            /// as we can not take value of same position more than once
            if(i == p1)
                p1 ++;
            else if(i == p2)
                p2--;
            ...
            if( sum < x )        /// now we need a larger value and that will be the left one as right one is the largest possible  (sorted array)
                ...
                pf("%d %d %d\n",input[i],input[p1],input[p2]);  /// print solution
                return 0;                                       /// code ends
            }
    ...
    if(i == n)
        pf("Got no such three values man sorry\n");
...
4 3 5 4 4
output
Got no such three values man sorry
5 1 10
3 4 5
*/
