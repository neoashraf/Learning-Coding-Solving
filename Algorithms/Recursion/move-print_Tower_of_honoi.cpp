...
int TOH(char a,char b,char c,int n){
    if(n==0)
        return 0;
    TOH(a,c,b,n-1);     ///from A using C take to B n-1 tiles
    printf("%c --> %c\n",a,c);  ///then we move from A to C
    TOH(b,a,c,n-1);     ///from B using A take to C n-1 tiles
}

int main() {
    int n;
    read(n);
    TOH('A','B','C',n);    ///from A using B take to C n tiles
    return 0;
}
