...
int fuck() {
    int i,j,n,b;
    read(n);
    for(b = 0;b<(1<<n);b++){   /// 1<<n represents 2^n
        for(i=0;i<n;i++){
            if(b &(1<<i))      /// 1<<i means what man ith power of i and what does that mean  ith bit is 1 rest is zero so
                              ///  if true it means ith bit is set and we can take that
                pf("%d ",i);
        }
        pf("\n");
    }
